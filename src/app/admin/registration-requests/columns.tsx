...
export const columns: ColumnDef<RegistrationRequest>[] = [
  {
    accessorKey: 'requested_at',
    header: getHeaderComponent,
    cell: ({ row }) => {
  ...
  },
  {
    ...
    cell: ({ row }) => {
      const request = row.original
      return <TrailingActions request={request} />
    },
  },
]

const TrailingActions = ({ request }: { request: RegistrationRequest }) => {
  const t = useTranslations('Admin.registration-requests.data-table.actions')

  const supabase = createBrowserClient()

  const updateRequest = async (status: RegistrationRequestStatus) => {
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end">
        ...
        <DropdownMenuItem>{t('view')}</DropdownMenuItem>
        <DropdownMenuItem onClick={() => updateRequest('approved')}>
          {t('approve')}
          <SmilePlus color="#33b918" className="ml-1 h-4 w-4" />
        </DropdownMenuItem>
        <DropdownMenuItem>
          {t('refuse')} <ShieldAlert className="ml-1 h-4 w-4" color="#f42a2a" />
        </DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
  )
